  ...
  config = mkIf cfg.enable {
    systemd.services.athens = {
      ...
      serviceConfig = {
        ...
        ReadWritePaths = mkIf (
          cfg.storage.disk.rootPath != null && (!hasPrefix "/var/lib/" cfg.storage.disk.rootPath)
        ) [ cfg.storage.disk.rootPath ];
        StateDirectory = mkIf (hasPrefix "/var/lib/" cfg.storage.disk.rootPath) [
          (removePrefix "/var/lib/" cfg.storage.disk.rootPath)
        ];
    ...
    networking.firewall = {
      allowedTCPPorts =
        optionals (cfg.unixSocket == null) [ cfg.port ]
