      ...
      let
        normalizedValue = lib.mapAttrs (
          ...
          if lib.isList val then
            forEach val (elem: if lib.isList elem then elem else [ elem ])
        ...
        mkValueString = concatMapStringsSep " " (
          v:
          let
            str = generators.mkValueStringDefault { } v;
          in
    ...
    networking.firewall = mkIf cfg.openFirewall {
      allowedTCPPorts =
        if cfg.upsd.listen == [ ] then
          ...
          unique (forEach cfg.upsd.listen (listen: listen.port));
    };
