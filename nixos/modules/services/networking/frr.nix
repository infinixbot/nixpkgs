...

{
  ...
  config = lib.mkIf (lib.any isEnabled allServices) {
    ...
    environment.etc =
      ...
      in
      (builtins.listToAttrs (map mkEtcLink (lib.filter isEnabled allServices)))
      // {
        "frr/vtysh.conf".text = "";
      };
          ...
          lib.nameValuePair daemon ({
            ...
            description =
              if service == "zebra" then
                ...
                "FRR ${lib.toUpper service} routing daemon";
            ...
            serviceConfig = {
              ...
              ExecReload =
                lib.mkIf (service != "mgmt")
                  "${pkgs.python3.interpreter} ${pkgs.frr}/libexec/frr/frr-reload.py --reload --daemon ${daemon} --bindir ${pkgs.frr}/bin --rundir /run/frr /etc/frr/${daemon}.conf";
              Restart = "on-abnormal";
            };
          });
      in
