    ...
    let
      ...
      configStr =
        if cfg.allowAuxiliaryImperativeNetworks then
          "-c /etc/wpa_supplicant.conf -I ${pkgs.writeText "wpa_supplicant.conf" generatedConfig}"
        else
...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    environment.etc."wpa_supplicant.conf" = lib.mkIf (!cfg.allowAuxiliaryImperativeNetworks) {
