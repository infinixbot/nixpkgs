...
let
  ...
  extras = {
    ...
    cl-cffi-gtk-gdk = pkg: { nativeLibs = [ gtk3 ]; };
    cl-cffi-gtk-gdk-pixbuf = pkg: { nativeLibs = [ gdk-pixbuf ]; };
    cl-cffi-gtk-pango = pkg: { nativeLibs = [ pango ]; };
    cl-cairo2 = pkg: { nativeLibs = [ cairo ]; };
    cl-cairo2-xlib = pkg: { nativeLibs = [ gtk2-x11 ]; };
    cl-devil = pkg: {
    ...
    mssql = pkg: { nativeLibs = [ freetds ]; };
    osicat = pkg: { LD_LIBRARY_PATH = "${pkg}/posix/"; };
    png = pkg: {
    ...
    trivial-ssh-libssh2 = pkg: { nativeLibs = [ libssh2 ]; };
    zmq = pkg: {
      nativeBuildInputs = [ pkgs.zeromq ];
      nativeLibs = [ pkgs.zeromq ];
    };
