    ...
    lib.attrsets.mapAttrsRecursive (
      ...
      if (builtins.elemAt path ((builtins.length path) - 1)) == "archives" then
        ...
        lib.warnIf (builtins.length validArchives == 0)
          "No valid archives for ${
            lib.concatMapStringsSep "." (x: ''"${x}"'') path
          } for os=${os}, arch=${arch}"
          (
            lib.optionals (builtins.length validArchives > 0) (
              lib.last (map (archive: (fetchurl { inherit (archive) url sha1; })) validArchives)
            )
          )
      else
        value
...
rec {
  ...
  emulator = lib.optionals includeEmulator (
