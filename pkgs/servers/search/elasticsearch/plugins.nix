...
in
{
    ...
    src = fetchurl {
      ...
      hash =
        if version == "7.17.16" then
          "sha256-wgm6N5fofs5wTM25ZT3dJkg7iDesXsc3Up419IAY9gk="
        else
  ...
  analysis-kuromoji = esPlugin rec {
    ...
    src = fetchurl {
      ...
      hash =
        if version == "7.17.16" then
          "sha256-SShdBcWfm21XoVhghSSiWIhsoXzG7wz6162iOmuf5EU="
        else
          throw "unsupported version ${version} for plugin ${pluginName}";
    };
    meta = with lib; {
  ...
  analysis-phonetic = esPlugin rec {
    ...
    src = fetchurl {
      ...
      hash =
        if version == "7.17.16" then
          "sha256-S/Cp9opeLitFh2/3Qw7/MFt6GcYKufxXKD6cJSi3SaQ="
        else
          throw "unsupported version ${version} for plugin ${pluginName}";
    };
    meta = with lib; {
  ...
  discovery-ec2 = esPlugin rec {
    ...
    src = fetchurl {
      ...
      hash =
        if version == "7.17.16" then
          "sha256-hMErTLd5fXg420Olz+j6Zv7WByA1aNq9FlEgCtkYIxY="
        else
          throw "unsupported version ${version} for plugin ${pluginName}";
    };
    meta = with lib; {
  ...
  ingest-attachment = esPlugin rec {
    ...
    src = fetchurl {
      ...
      hash =
        if version == "7.17.16" then
          "sha256-z0gfdx98urCzdQNlVn99CmteG6jweOmUDmGJW89twtU="
        else
          throw "unsupported version ${version} for plugin ${pluginName}";
    };
    meta = with lib; {
  ...
  repository-s3 = esPlugin rec {
    ...
    src = fetchurl {
      ...
      hash =
        if version == "7.17.16" then
          "sha256-TWMN8jzFjzBVTUB+zn4tJr47VMXHC8U+014BvnArK8M="
        else
          throw "unsupported version ${version} for plugin ${pluginName}";
    };
    meta = with lib; {
  ...
  repository-gcs = esPlugin rec {
    ...
    src = fetchurl {
      ...
      hash =
        if version == "7.17.16" then
          "sha256-hG5wy1Xw4T1NzI7pja3CejwJg002/n6YqM1/QaVSWbg="
        else
          throw "unsupported version ${version} for plugin ${pluginName}";
    };
    meta = with lib; {
